...
def train():
    ...
    env = Grid(x_size=4, y_size=4, n_agents=2, fov_x=3, fov_y=3)
    ...
    for episode in range(train_episodes):
        ...
        joblib.dump(q_class, "q_class_multi_fixed_small_fov.txt", compress=3)
    return q


def get_drone_positions(obs, agent_pos):
    return "{" + "1 |-> ({0} |-> {1}), 2 |-> ({2} |-> {3})".format(agent_pos[0][0], agent_pos[0][1], agent_pos[1][0], agent_pos[1][1]) + "}"

# initialize the environment
n_agents = 2
env = Grid(x_size=4, y_size=4, n_agents=n_agents)
